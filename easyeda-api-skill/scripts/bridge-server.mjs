...
// Node 24 resolves this installed ws build as CommonJS, so use its default
// export instead of a named ESM import. This remains compatible with ws 8.
...
const SAFE_MODE = process.env.EASYEDA_BRIDGE_SAFE_MODE !== 'false';
const CODEX_COMPANION_OPS = new Set([
  'library.search',
  'project.health', 'project.createDesign', 'project.openLinkedDocument', 'project.current', 'project.backupPcb',
  'schematic.exportNetlist', 'schematic.exportNativeNetlist', 'schematic.validateNetlist', 'schematic.drc',
  'schematic.placeComponents',
  'schematic.createWires',
  'netlist.compare', 'pcb.summary', 'pcb.importChanges', 'pcb.netLengths', 'drc.run', 'dfm.runJlc',
  'pcb.placeComponents',
  'pcb.assignNets',
  'pcb.applyNetlist',
  'pcb.createTracks',
  'route.preflight', 'route.exportDsn', 'route.runExternalRouter', 'route.importSes', 'route.verify',
  'rules.detectDiffPairs', 'rules.auditHighSpeed', 'rules.applyDiffPairs', 'locate.violation',
]);
...
function isSafeCodexRpcCode(code) {
  const match = /^return await eda\.sys_MessageBus\.rpcCallPublic\("easyeda-codex\.rpc\.v8",([\s\S]+),(\d+)\);\s*$/.exec(code);
  ...
  try {
    ...
    return Boolean(
      ...
      && CODEX_COMPANION_OPS.has(request.op)
...
const httpServer = createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/execute') {
    ...
    try {
      ...
      if (SAFE_MODE && !isSafeCodexRpcCode(code)) {
        res.writeHead(403, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ success: false, error: 'Bridge safe mode only allows easyeda-codex.rpc requests.' }));
        return;
      }
      ...
      const status = /no EDA window connected|not connected/i.test(message) ? 503 : 500;
  ...
  if (clientType === 'eda') {
    ...
    ws.on('message', async (raw) => {
      try {
        const msg = JSON.parse(raw.toString());
        if (msg.type === 'execute') {
          if (SAFE_MODE && !isSafeCodexRpcCode(msg.code)) {
            ws.send(JSON.stringify({
              type: 'error',
              id: msg.id,
              error: 'Bridge safe mode only allows easyeda-codex.rpc requests.',
              timestamp: Date.now(),
            }));
            return;
          }
...
${formatBannerLine('Safe Mode', SAFE_MODE ? 'structured Codex RPC only' : 'disabled')}
║                                                              ║
